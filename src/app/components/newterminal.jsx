  ...
  const parsedUserDetails = JSON.parse(decodedUserDetails)
    ...
    const placeTradeFunction = async () => {
      try {
        ...
        if(marginSum < dynamicBalance && symbolPrice!==null && leverageValue>0){
          ...
          const matchingTradeIds = pendingTrades
            .filter((trade) => {
              if (trade.buyOrSell === "buy") {
                ...
                return symbolPrice >= trade.openingprice && trade.openingprice >= symbolPrice - PRICE_BUFFER;
...
useEffect(() => {
  ...
  openTrades.forEach((trade) => {
    ...
    } else if (trade.takeProfitValue && profitOrLoss >= trade.takeProfitValue) {
      closeTradeFunc(trade.id, profitOrLoss);
    }
  });
...
const closeTradeFunc = async (tradeId, profitOrLoss) => {
  try {
    ...
    if (
      ...
      console.warn("Invalid data passed to closeTradeFunc. Skipping trade closure.");
      return;
    }
    ...
    setTrades((prevTrades) =>
      prevTrades.map((trade) =>
        trade.id === tradeId
          ? { ...trade, profitOrLoss, closingPrice: symbolPrice, closingTime: new Date() }
          : trade
      )
    );
...
useEffect(() => {
  // Update open trades
  const newOpenTrades = trades.filter((trade) => trade.closingTime === null && trade.pending === false);
  setopenTrades(newOpenTrades);
}, [isTradeActive, isTradeClosed, selectButton, activeButton, trades]);

useEffect(() => {
  // Update closed trades
  const newClosedTrades = trades.filter((trade) => trade.closingTime !== null && trade.pending === false);
  setclosedTrades(newClosedTrades);
}, [isTradeClosed, trades, isTradeActive]);

useEffect(() => {
  // Update pending trades
  const newPendingTrades = trades.filter((trade) => trade.closingTime === null && trade.pending === true);
...
useEffect(() => {
  const getInitialDemoBalance = async () => {
    try {
      const response = await axios.get(`/api/changeDemoBalance?username=${parsedUserDetails.username}`);
      if (response.status === 200) {
        let fixedBalance = 0;
        if (response.data.category === "beginner" || response.data.category === "null") {
          fixedBalance = 10000;
    ...
    <div className="containerOuter">
      ...
      <div className=" w-[100vw] h-[40px] lg:h-[8vh] bg-zinc-950 border-b-4 border-slate-600 text-white flex justify-between items-center">
        ...
        <div className="font-bold flex justify-center gap-x-1">
          <div className="text-xl font-thin">$</div> {dynamicBalance !== null ? (Number(dynamicBalance).toFixed(3)) : ' '}
          </div>
      ...
      <div className="h-[20vh] w-[100%] bg-zinc-950 text-white  border-slate-600 flex flex-col">
        <div className="flex text-base font-thin text-slate-500 justify-start h-[30%] w-[40%]">
          <button
            ...
            className={`w-[120px] ${activeButton === 1 ? "bg-slate-800 text-white" : "bg-slate-900"} hover:bg-slate-800 border-r-2 border-slate-600`}
          >
            Open ({openTrades.length})
          </button>
          <button
            onClick={() => handleButtonClick(2)}
            className={`w-[120px] ${activeButton === 2 ? "bg-slate-800 text-white" : "bg-slate-900"} hover:bg-slate-800 border-r-2 border-slate-600`}
          >
          ...
          {activeButton !== null && (
            <div className="overflow-y-auto h-full max-h-[190px] custom-scrollbar overflow-x-auto w-full">
              ...
              <div className="flex bg-gray-900 text-slate-400 text-[12px] h-8 sticky top-0 z-10 gap-x-2 w-full">
                <div className="w-[70px] p-2   text-center flex-shrink-0">Symbol</div>
                <div className="w-[70px]  p-2 text-center flex-shrink-0">Sell/Buy</div>
                <div className="w-[70px] p-2 text-center flex-shrink-0">Volume</div>
                <div className="w-[70px]  p-2 text-center flex-shrink-0">Margin</div>
                <div className="w-[85px]  p-2 text-center flex-shrink-0">OpeningPrice</div>
                {activeButton === 3 && (
                  <div className="w-[85px] p-2   text-center flex-shrink-0">ClosingPrice</div>
                )}
                {(activeButton === 1 || activeButton === 2) && (
                  <div className="w-[85px]  p-2  text-center flex-shrink-0">CurrentPrice</div>
                )}
                {(activeButton === 1 || activeButton === 3) && (
                  <div className="w-[70px]  p-2 text-center flex-shrink-0">Profit</div>
                )}
                {activeButton === 1 && (
                  <div className="w-[85px] p-2  text-center flex-shrink-0">CloseTrade</div>
                )}
                {(activeButton === 1 || activeButton === 3) && (
                  <div className="w-[115px] p-2 text-center flex-shrink-0">OpeningTime</div>
                )}
                {activeButton === 3 && (
                  <div className="w-[115px]   p-2 text-center  ">ClosingTime</div>
                )}
                ...
                .map((row, index) => {
                  ...
                  return (
                    <div key={index} className="flex gap-x-2">
                      <div className="w-[70px] p-2 text-center">{row.symbol}</div>
                      <div className="w-[70px] p-2 text-center">{row.buyOrSell}</div>
                      <div className="w-[70px] p-2 text-center">{row.unitsOrLots}</div>
                      <div className="w-[70px] p-2 text-center">{row.margin}</div>
                      <div className="w-[85px] p-2 text-center ">{row.openingprice}</div>
                      <div className="w-[85px] p-2 text-center ">{symbolPrice}</div>
                      ...
                      {row.takeProfitValue === null ? (
                        <div className="w-[70px] p-2 text-center">- - - -</div>
                      ) : (
                        <div className="w-[70px] p-2 text-center ">{row.takeProfitValue}</div>
                      )}
                      {row.stopLossValue === null ? (
                        <div className="w-[70px] p-2 text-center">- - - -</div>
                      ) : (
                        <div className="w-[70px] p-2 text-center">{row.stopLossValue}</div>
                      )}
                    </div>
                  );
                })}
  ...
  closedTrades.length === 0 && activeButton === 3 ? (
    ...
    <div>
      {closedTrades
        ...
        .map((row, index) => {
          ...
          return (
            <div key={index} className="flex gap-x-2 ">
              ...
              <div className="w-[70px] p-2 text-center">{row.margin}</div>
              <div className="w-[85px] p-2 text-center">{row.openingprice}</div>
              <div className="w-[85px] p-2 text-center ">{row.closingPrice}</div>
              <div className="w-[70px] p-2 text-center">
                <span className={` ${row.profitOrLoss < 0 ? "text-red-500" : "text-green-600"}`}>
        ...
        <p className={` text-slate-300 p-2  text-base ${symbol ? "" : "invisible"}`}>
      ...
      <div className="w-[25%] flex flex-col text-2xl ml-4 font-semibold mr-1">
        <button onClick={handleIncrease} className="ring-2 text-[#FEEC37] mb-1 hover:bg-slate-700">+</button>
        <button onClick={handleDecrease} className="ring-2 text-[#6EC207] hover:bg-slate-700 ">-</button>
      </div>
    </div>
